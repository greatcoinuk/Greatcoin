...
#if defined(NDEBUG)
# error "Greatcoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Greatcoin Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Greatcoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// Greatcoin: find block wanted by given orphan block
...
// miner's coin base reward newtodo1
int64_t GetProofOfWorkReward(int nHeight,int64_t nFees)
{
    int64_t nSubsidy = 20 * COIN;

    if(nHeight <= 10)
        nSubsidy += 9660000 * COIN;
...
// miner's coin stake reward newtodo1
...
// Greatcoin: find last block index up to pindex
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal Greatcoin miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Greatcoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Greatcoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // Greatcoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // Greatcoin: track money supply and mint amount info
...
// Greatcoin: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // Greatcoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // Greatcoin: compute stake entropy bit for stake modifier
    ...
    // Greatcoin: compute stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Greatcoin: check proof-of-stake
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // Greatcoin: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// Greatcoin: attempt to generate suitable proof-of-stake
...
{
    RenameThread("Greatcoin-loadblk");
